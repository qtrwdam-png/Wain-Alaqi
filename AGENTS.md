...
- **Robots:** `src/app/robots.ts` (Next.js convention → auto-generates `/robots.txt`). Allows all crawlers on `/`, disallows `/admin/`, `/dashboard/`, `/account/`, `/login`, `/register`, `/unauthorized`, `/api/`. Declares the sitemap URL.
